...
public:
    ...
    const Word *
    nextWord(const std::vector<const Word *> &sentence, unsigned long n, const std::stack<const Word *> &markerStack,
             const std::vector<const Word *> topic, bool finishSentence = false, bool debug = false) const;

    const Gram *
    nextGram(const std::vector<const Word *> &sentence, unsigned long n, const std::stack<const Word *> &markerStack,
             const std::vector<const Word *> topic, bool finishSentence = false, bool debug = false) const;
