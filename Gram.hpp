...
public:
    ...
    const Gram *
    next(const std::vector<const Word *> &sentence, unsigned long position, const std::stack<const Word *> &markerStack,
    ...
    void fromJson(const Json::Value gram_json, std::map<unsigned long, std::unique_ptr<Word>> &wordsById);
